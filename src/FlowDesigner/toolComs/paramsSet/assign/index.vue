...
import { Button, Flex, Form } from 'ant-design-vue'
import type { StructuredCondition } from 'src/LogicFlow/types'
import { DragList } from '../../../resource/components'
import { getFlowNode, getFlowParentNode } from '../../../utils/flowTree'
...
<template>
  <div class="field-set-wrapper">
    ...
    <template v-if="true">
      <DragList v-if="tempValues.length > 0" :show-add-icon="false" :show-del-icon="false" :list="tempValues"
        height="auto" @update:list="triggerUpdate">
        <template #itemRender="{ record }">
          <FieldSetItem :row="record" :rowList="tempValues" @update:value="(val: any) => updateRow(record.id, val)"
            @on-delete="handleDel(record.id)" @on-add="(type: 'brother' | 'child') => handleAdd(type, record.id)" />
        </template>
      </DragList>
