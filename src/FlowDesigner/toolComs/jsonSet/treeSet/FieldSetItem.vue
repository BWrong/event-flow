...
import { Dropdown, Flex, Input, Menu, MenuItem, Select, Space } from 'ant-design-vue'
...
import { defaultValues } from '../../../utils/resolveDs'
...
<template>
  <Flex gap="small">
    <Input v-if="showTargetField" class="shrink-0 basis-150px!" :disabled="'array' === row.pType" :value="row.target"
      placeholder="参数名" @change="(e: any) => handleChange('target', e.target.value)" />

    <span v-if="showTargetField" class="shrink-0"> : </span>
    <div :class="showTargetField ? 'basis-150px! shrink-0' : 'basis-342px! shrink-0'">
      <!-- 这里区分下数组对象，单独显示 -->
      <Input v-if="['array', 'object'].includes(row.targetDataType?.type)" :disabled="true" :value="row.source"></Input>
      <template v-else>
        <!-- 普通类型值 -->
        <WidgetItem v-if="row.sourceType && row.sourceType === 'value'" :value="row.source"
          :cur-page-field="curPageField" need-valid @update:value="handleRightChange" />
      </template>
    </div>

    <!-- 类型选择 -->
    <Select style="flex-shrink: 0;flex:0 0 150px" :value="row.targetDataType?.type" :options="fullFieldTypeOptions"
      placeholder="请选择" @change="handleTypeChange" />
    <Space>
      <!-- 部分操作项配置 -->
      ...
      <Dropdown>
        <template #overlay>
          <Menu>
            ...
            <MenuItem v-if="['object', 'array'].includes(row.targetDataType?.type)" key="child"
