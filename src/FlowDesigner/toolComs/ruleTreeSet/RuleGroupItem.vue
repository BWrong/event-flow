...
<template>
  <div>
    <!-- 多项 -->
    <div v-if="row.id && row.criteriaList && row.criteriaList.length > 0" class="rule-set-container">
      <!-- 为父节点 -->
      <div class="rule-list-container">
        <div class="rule-group-wrapper">
          <div class="rule-relation-wrapper">
            <div class="rule-line" />
            <a-select style="width: 60px" class="rule-trigger" :value="row.combineType" :options="combineTypeOptions"
              @change="handleChange" />
          </div>
          <!-- {{ props.row.rules }} -->
          <!-- 具体规则项列表 -->
          <div :class="['rule-list-wrapper', 'has-child']">
            <rule-group-item v-for="(rule, index) in row.criteriaList" :key="rule.ruleId" :row-index="index"
              :row="rule" />
          </div>
        </div>
      </div>
    </div>
    <rule-item v-else :row-index="rowIndex" :row="row" />
  </div>
</template>
