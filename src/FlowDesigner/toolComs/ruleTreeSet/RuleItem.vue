<template>
  <a-flex gap="small" style="padding: 12px" align="center">
    <!-- 左侧变量选择 -->
    <field-tree-select style="flex: 0 0 150px" class="overflow-hidden-one" :value="row.target"
      @update:value="handleLeftChange" />
    <!-- 连接器 -->
    <a-select :value="row.operator" style="flex: 0 0 90px" :options="relaOptions" placeholder="请选择"
      :dropdown-match-select-width="false" @change="handleOptChange" />
    <template v-if="needSetValue">
      <a-select style="flex: 0 0 90px" :value="row.sourceType" :options="sourceTypeOptions" placeholder="请选择"
        @change="(val: string) => commonSetValue({ sourceType: val })" />
      <!-- 动态组件显示 -->
      <widget-item style="width: auto" v-if="row.sourceType && row.sourceType === 'value'" :value="row.source"
        :disabled="!row.target" :cur-page-field="row" @update:value="handleRightChange" />
      <!-- 字段选择显示 -->
      <field-tree-select v-if="row.sourceType && row.sourceType === 'variable'" :value="row.source"
        class="overflow-hidden-one" :filter-field-types="filterFieldTypes" @update:value="handleRightVarChange" />
      <!-- 公式选择显示 -->
      <FormulaSelect v-if="row.sourceType && row.sourceType === 'formula'" :value="{
...
import { CheckCircleFilled, CloseCircleFilled, MinusOutlined, PlusOutlined } from '@ant-design/icons-vue'
import { computed, inject, provide, watch } from 'vue'
import { sourceTypeOptions } from '../../resource/dicts'
import { COMMON_CONDITION, fieldTypeMap, notSupConstParamTypes, type FieldType } from '../../resource/dicts/fields'
import type { StructuredCondition } from '../../types'
import { getEditorNodeId } from '../../utils/helper'
