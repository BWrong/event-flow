...
import { type Ref, inject, type ComputedRef, unref, ref, computed } from 'vue'
import { to } from 'await-to-js'
import { cloneDeep } from 'lodash-es'
import { guid } from '../../../common/helper'
import type { TreeProps } from 'ant-design-vue'
import { designerStoreInjectKey } from '../../../designer/injectKeys'
import { type FlowForNodeProps } from '../../for'
import { getFlowParentNode, flatFlowTreeDataClosure, findFlowParentIds } from '../../../common/flowTree'
import type { TreeVariableItem, FlowNode, IDataType, FlowVariables } from '../../../types'
...
export const useFindTreeVars = (curNode: FlowNode, extraOptions?: ComputedRef<TreeNodeOption>) => {
    ...
    const generatorItem = (arr: IDataType[]) => {
      ...
      if (arr.length > 0) {
        arr.forEach((item: IDataType, index: number) => {
          ...
          // 兼容首层的情况
          const itemTypes = item?.itemTypes || item?.typeInfo?.itemTypes || item?.dataType?.itemTypes
          const type = item?.type || item?.typeInfo || item?.dataType?.type
          const reference = item?.reference || item?.typeInfo?.reference || item?.dataType?.reference
          ...
          if (item.name) {
            // 判断过滤变量
            if (newOptions?.value?.filterFieldTypes && newOptions?.value?.filterFieldTypes.length > 0) {
  ...
  const resultTreeData = computed<TreeVariableItem[]>(() => {
    ...
    return [...transformVar(cloneDeep(curToRootVarList.value) as TreeVariableItem[]), ...varTreeData.value]
  })
  // 平铺结构（整合下父容器的）
