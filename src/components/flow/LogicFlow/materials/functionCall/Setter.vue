...
<template>
  <AForm layout="vertical">
    ...
    <AFormItem label="参数配置">
      <ATable
        :columns="columns"
        :data-source="formState.functionParams"
        size="small"
        bordered
        :pagination="false"
      >
        <template #bodyCell="{ column, record }">
          ...
          <template v-if="column.dataIndex === 'value' && validateInfos">
            <ValidItem
              :validate-info="
                modelRef?.sourceType === 'formula' ? validateInfos.expression : validateInfos.source
              "
            >
              <div class="min-w-0 flex-1">
                <WidgetItem
                  v-if="record.type === 'value'"
                  :value="record.value"
                  :cur-page-field="record"
                  @update:value="(val: string) => triggerValues({ source: val })"
                />
                <!-- 字段选择显示 -->
                <FieldTreeSelect
                  v-if="record.type === 'variable'"
                  :value="record.value"
