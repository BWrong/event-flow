<template>
  <div class="get-storage-config-wrapper">
    ...
    <ATable
      :columns="columns"
      :data-source="formState.storageData"
      :pagination="false"
    >
      <template #bodyCell="{ column, record }">
        <template v-if="column.dataIndex === 'variable'">
          ...
          <div style="width: 200px;">
            <FieldTreeSelect
              class="flex"
              :value="record[column.dataIndex].variableValue"
        ...
        <template v-if="column.dataIndex === 'storageKey'">
          <SelectValue
            v-model:method="record[column.dataIndex].method"
...
<script setup lang="ts">
  ...
  import SelectValue from '../toolComs/selectValue/index.vue'
