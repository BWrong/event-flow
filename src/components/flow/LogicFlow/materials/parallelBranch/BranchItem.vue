...
import { FlowBranchRouter, FlowMaterial, FlowNode } from '../../types'
import { computed, inject, ref } from 'vue'
import FlowNodeHandler from '../../designer/FlowNodeHandler.vue'
import { Popconfirm as APopconfirm, message, Input as AInput, Popover as APopover } from 'ant-design-vue'
import { designerStoreInjectKey, themeInjectKey } from '../../designer/injectKeys'
...
function handleMoveLeft() {
  let children = props.parentData.children || []
  if (props.index > 0) {
...
<template>
  <div class="flow-node-branch-router">
    ...
    <div class="flow-node-branch-condition" :id="data.id">
      <div
        class="flow-node-condition-card flow-node-box"
        @click.stop="handleEditCondition"
        :class="{
          selected: designerStore.state.setterProps?.branchRouter?.id === data.id,
          error: checkInfo?.type === 'error',
          warning: checkInfo?.type === 'warning'
        }"
      >
        <div class="flow-node-condition-header">
          <div @click.stop>
            <AInput
              v-if="editingTitle"
              class="flow-node-condition-title edit-input"
              v-model:value="editTitleValue"
              allow-clear
              v-focus
              size="small"
              @blur="handleSaveTitle"
              @press-enter="handleSaveTitle"
            />
          ...
          <span class="flow-node-condition-priority">
            <template v-if="parentData.props.isParallel"> 并行 </template>
            <template v-else> 优先级{{ index + 1 }} </template>
          </span>
        </div>
      ...
      <template v-if="!data.props.isDefaultBranch">
        ...
        <div class="flow-node-toolbar">
          <a-popover
            title="配置错误"
            :get-popup-container="(el) => el.parentElement.parentElement"
          ...
          <a-popconfirm
            ...
            okType="danger"
            v-if="parentData.children.length > 2"
          >
