 ...
 * @returns
 */
export function generateTypeDescription(
  data: any,
  name = '',
): { type: string; name: string; itemTypes?: any[] } {
  const type = getValueType(data)
  if (type === 'object') {
    const itemTypes = Object.keys(data).map((key) => generateTypeDescription(data[key], key))
    return { type: 'object', name, itemTypes }
  } else {
    return { type, name }
  }
}
