...
import { Flex, Form, Select, Space } from 'ant-design-vue'
import type { Rule } from 'ant-design-vue/es/form'
import type { StructuredCondition } from 'src/LogicFlow/types'
import { computed, reactive, watch } from 'vue'
import { ValidItem } from '../../../resource/components'
import { fieldTypeMap, sourceTypeOptions, type FieldType, type ISourceType } from '../../../resource/dicts'
  ...
  <div :class="['field-set-item-wrapper']">
    <Flex gap="middle">
      <ValidItem :validate-info="validateInfos.target">
        <!-- 会出现警告？ -->
        <a-form-item-rest>
          <FieldTreeSelect class="overflow-hidden-one" style="flex: 0 0 25%"
            :status="validateInfos.target.validateStatus" :value="modelRef.target"
            @update:value="handleTargetVarChange" />
        </a-form-item-rest>
      </ValidItem>
      <!-- 连接器 -->
      <Select :value="modelRef.operator" style="flex: 0 0 20%" :options="relaOptions" placeholder="请选择" disabled
        :dropdown-match-select-width="false" />
      <Select class="overflow-hidden-one" style="flex: 0 0 15%" :value="modelRef.sourceType"
        :options="Array.from(sourceTypeOptions)" placeholder="请选择" @change="handleSourceTypeChange" />
      <ValidItem :validate-info="modelRef.sourceType === 'formula' ? validateInfos.expression : validateInfos.source">
        <div class="min-w-0 flex-1">
          <WidgetItem v-if="modelRef.sourceType && modelRef.sourceType === 'value'" :value="modelRef.source"
            :cur-page-field="modelRef" @update:value="(val: string) => triggerValues({ source: val })" />
          <!-- 字段选择显示 -->
          <FieldTreeSelect v-if="modelRef.sourceType && modelRef.sourceType === 'variable'" :value="modelRef.source"
