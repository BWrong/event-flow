<template>
  <!-- 含有子项，这里仅处理结构体 -->
  <a-sub-menu :key="menuInfo.key" :title="menuInfo.name" v-if="menuInfo.children && menuInfo.children.length > 0">
    <template v-for="item in menuInfo.children" :key="item.name">
      <sider-item :menuInfo="item"></sider-item>
    </template>
  </a-sub-menu>
  <a-menu-item :title="menuInfo.name" :key="menuInfo.key" @mouseenter="handleMouseenter(menuInfo)" v-else>
