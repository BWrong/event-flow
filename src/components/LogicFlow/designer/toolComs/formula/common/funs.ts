...
const textList: IFormulaFunctionListItem[] = [
  ...
  },
  {
    name: 'TEXT',
    code: 'TEXT()',
    desc: '<i>TEXT</i>函数将数字转为中文大写<br/>用法：<i>TEXT</i>(数字)<br/> 示例：<i>TEXT</i>(123)返回"壹佰贰拾叁"',
  },
  {
    name: 'REPLACE',
    code: 'REPLACE()',
...
const logicListDefault: IFormulaFunctionListItem[] = [
]
const advancedList: IFormulaFunctionListItem[] = [
]
const advancedListDefault: IFormulaFunctionListItem[] = [
  {
    name: 'GETUSERNAME',
    code: 'GETUSERNAME()',
    desc: '<i>GETUSERNAME</i>函数可以获取当前用户的昵称<br/>用法：<i>GETUSERNAME</i>()<br/>示例：略',
  },
  {
    name: 'INDEX',
    code: 'INDEX()',
  ...
  },
  {
    name: 'MAPX',
    code: 'MAPX()',
    desc: '<i>MAPX</i>用来进行跨表单取数的高级函数。<br/>用法：<i>MAPX</i>(operation,map_value,map_field,result_field)<br/>示例：略',
  },
  {
    name: 'RECNO',
    code: 'RECNO()',
    desc: '<i>RECNO</i>依据当前表单被新打开的次数进行不断累计，起始值为1。<br/>用法：<i>RECNO</i>()<br/>示例：略',
  },
  {
    name: 'UUID',
    code: 'UUID()',
    desc: '<i>UUID</i>随机码生成器。可适用于随机流水号的使用场景等。<br/>用法：<i>UUID</i>()<br/>示例：略',
  },
]

// 带Default名称的为表单中使用, Factory数据工厂中使用，未带的 数据工厂｜表单 公用
export {
  mathList,
  mathListDefault,
  mathListFactory,
