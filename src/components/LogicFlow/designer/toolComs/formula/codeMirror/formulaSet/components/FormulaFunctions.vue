...
import { Input, Popover, Tree } from 'ant-design-vue';
...
<template>
  <div class="formula-functions">
    <div class="search-header">
      <Input placeholder="搜索函数" allow-clear :bordered="false" v-model:value="searchValue">
      <template #prefix>
    ...
    <Tree v-if="filteredFields.length > 0" class="function-list" :field-names="{ label: 'name', key: 'code' }"
      :tree-data="filteredFields" :expanded-keys="expandedKeys" :auto-expand-parent="autoExpandParent"
      @expand="onExpand" @select="handleSelectChange">
      <template #title="item">
        <!-- 字段渲染-->
        <template v-if="!item.children">
          <Popover :title="item.name" placement="left" v-if="showPopup">
            <template #content>
              <div class="formula-desc">
                <main v-html="item.desc" />
              </div>
            </template>
            <div class="function-item" :title="item.name">
              <span class="name overflow-hidden-one" :title="item.name">
                {{ item.name }}
              </span>
            </div>
          </Popover>
          <div v-else class="function-item" :title="item.name" @mouseenter="handleEnter(item)">
            <span class="name overflow-hidden-one" :title="item.name">
              {{ item.name }}
            </span>
          </div>
