...
import type { CodeMirrorValue, Mark } from '../../types'
...
const setMark = (item: Mark) => {
  const { name, editorName, value, extra, from, to, className } = item
  const showName = editorName || name // 兼容显示
  const widget = document.createElement('span')
  widget.innerHTML = showName
  widget.className = className || 'my-snippet'
  ...
  CodeMirrorEditor.markText(from, to, {
    className: className || 'my-snippet',
    atomic: true,
    replacedWith: widget,
    attributes: extra
      ? {
...
const transformMarks2Str = (valueObj: { text: string; marks: Record<string, any> }) => {
  ...
  if (CodeMirrorEditor && valueObj) {
    ...
    const textValue = valueObj.text
    const replacements = marks.map((mark: any) => {
      const { from, to, markType, name } = mark
      let text = ''
      if (markType) {
        if (markType === 'field') {
