<template>
  <!-- 含有子项，这里仅处理结构体 -->
  <ASubMenu :key="menuInfo.key" :title="menuInfo.name" :icon="getIconNode(menuInfo)"
    v-if="menuInfo.children && menuInfo.children.length > 0">
    <template v-for="item in menuInfo.children" :key="item.name">
      <sider-item :menuInfo="item" :filterFieldTypes="filterFieldTypes"
        :allowParentSelect="allowParentSelect"></sider-item>
    </template>
  </ASubMenu>
  <!-- 区分是否是ref结构体 -->
