<template>
  <div class="open-page-config-wrapper">
    ...
    <AFormItem label="打开页面类型">
      <ARadioGroup v-model:value="formState.pageType" @change="onPageTypeChange">
        <ARadio value="1">应用内页面</ARadio>
        <ARadio value="2">返回上一页面</ARadio>
        <ARadio value="3">其它链接</ARadio>
      </ARadioGroup>
    </AFormItem>
    <AFormItem v-if="formState.pageType == 1" label="打开目标页面">
      <ASelect v-model:value="formState.targetPageAddress" :options="pageList"
        :fieldNames="{ label: 'description', value: 'route' }" />
    </AFormItem>
    <AFormItem v-if="formState.pageType == 3" label="页面链接(URL)">
      <div style="display: flex;">
        <ASelect v-model:value="formState.pageUrlMethod" :options="pageUrlOption"
          style="width: 49%;margin-right: 20px;" />
        <!-- 表达式 -->
        <AButton v-if="formState.pageUrlMethod == 'expression'"
          :class="['flex', formState.content.expression ? 'primary' : '']" @click="visible = true">
          {{ formState.content.expression ? '编辑表达式' : '设置表达式' }}
        </AButton>

        <!-- 变量 -->
        <FieldTreeSelect v-if="formState.pageUrlMethod == 'variable'" class="flex" :value="formState.pageUrlVariable"
          @update:value="onFieldSelect" />
        <!-- 值 -->
        <AInput v-if="formState.pageUrlMethod == '' || formState.pageUrlMethod == 'value'" class="flex"
          v-model:value="formState.pageUrlValue" />
      </div>
    </AFormItem>
    <AFormItem v-if="formState.pageType == 1 || formState.pageType == 3" label="打开方式(仅PC端)">
      <ASelect v-model:value="formState.openMode" :options="openOption" />
    </AFormItem>
  </div>
  <AModal v-if="visible" v-model:open="visible" title="表达式" cancelText="取消" okText="确定" :onOk="handleOk"
