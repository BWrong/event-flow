...
import { QuestionTip } from '../../designer/resource/components'
import { useFindTreeVars } from '../../designer/resource/hooks/useFindTreeVars'
import { FieldTreeSelect } from '../../designer/toolComs'
import type { FlowVariables, IDataType, TreeVariableItem, VariableType } from '../../types'
...
<template>
  <div class="for-node-config-wrapper">
    ...
    <AFormItem :name="['props', 'loopCollection', 'name']" class="normal-label"
      :rules="rulesRef['loopCollection.name']">
      <template #label>
        <QuestionTip label="集合变量" tip="当前节点需要循环处理的集合类型变量"></QuestionTip>
      </template>
      <field-tree-select :value="formState.loopCollection.name" :filterFieldTypes="['array']" :allowParentSelect="false"
    ...
    <a-row :gutter="20">
      <a-col :span="12">
        <AFormItem :name="['props', 'loopEntry']" class="normal-label" :rules="rulesRef.loopEntry">
          ...
          <a-input placeholder="请输入" :value="formState.loopEntry" @change="handleLoopEntryChange"
      ...
      <a-col :span="12">
        <AFormItem :name="['props', 'loopSubscript']" class="normal-label">
          ...
          <a-input placeholder="请输入" :value="formState.loopSubscript" @change="handleLoopSubscriptChange"
