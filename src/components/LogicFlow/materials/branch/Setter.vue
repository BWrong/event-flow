...
<template>
  <div class="branch-item-node-config-wrapper">
    ...
    <a-alert v-else class="mb-3" :message="formState.props.conditionType === 'structured'
      ? '当满足以下规则条件时，流程进入当前分支。'
    ...
    <template v-if="!formState.props.isDefaultBranch">
      <AFormItem name="conditionType" label="条件配置方式">
        <a-radio-group v-model:value="formState.props.conditionType" :options="[
          { label: '结构化', value: 'structured' },
          { label: '表达式', value: 'expression' }
        ]" @change="triggerNode"></a-radio-group>
      </AFormItem>
...
const triggerNode = () => {
  ...
  if (!tempNode.props.conditions) {
    tempNode.props.conditions = []
  }

  // 同时更新 children 和 conditions
  tempNode.children[branchIndex.value] = {
    ...tempNode.children[branchIndex.value] || {},
    ...formState.value
  }
  tempNode.props.conditions[branchIndex.value] = {
    ...tempNode.props.conditions[branchIndex.value] || {},
    ...formState.value.props
  }
  ...
  (val: FlowBranchRouter) => {
    if (props.node) {
      ...
      if (childrenNodes.length > 0 && branchRouter.value) {
        ...
        formState.value = childrenNodes[branchIndex.value] || createBranchNodeData()
      }
    }
  },
  { immediate: true }
